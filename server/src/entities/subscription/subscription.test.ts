...
describe('subscribe', () => {
  ...
  it('should unsubscribe neighborhood', async () => {
    ...
    const res = await fakeSubscriptions.unsubscribe('1234', 'Botafogo');
    expect(res[0].userId).toBe('1234');
    expect(res[0].neighborhoodId).toBe('Botafogo');
  });
  ...
  it('should increment unread subscriptions', async () => {
    ...
    expect(fakeSubscriptions.subscriptions[1].unread).toBe(3);
    expect(fakeSubscriptions.subscriptions[0].unread).toBe(0);
  });
  ...
  it('should set unread subscriptions to zero', async () => {
    ...
    expect(fakeSubscriptions.subscriptions[0].unread).toEqual(0);
  });
});
