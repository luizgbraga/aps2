...
export default function MapComponent() {
    ...
    const addCurrentLocationButton2 = (map: google.maps.Map) => {
    ...
    useEffect(() => {
        if (map) {
            addCurrentLocationButton2(map);
            setMapToCurrentLocation(map);
        }
    }, [map]);
    return <>
        {!map && <Spin />}
        <div ref={ref} style={{ height: 'calc(100vh - 144px)', width: '100%', display: map ? 'flex' : 'none' }}>
        </div>
    </>;
}
